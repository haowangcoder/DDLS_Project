...
each request is fixed at the gateway and every client must perform inference
using **only its locally stored home-cluster LoRA expert**.

## Overview

The training procedure performs two structurally distinct forward passes on
every batch and combines them with a single backward step:

- **Home pass** activates only the home-cluster expert; produces the standard
  task loss `L_spec`.
- **Non-home pass** deactivates the home and universal experts; uniformly
  pools the remaining `K-1` cluster experts; produces the cross-task loss
  `L_xc`.
- **Combined objective** `L_total = L_spec + λ · L_xc` with `λ = 0.20`.

A single backward step on `L_total` routes the home-task gradient exclusively
into the home-cluster expert, while the cross-task gradient propagates through
the non-home cluster experts and the shared classifier head. The matching
server aggregation rule weights each cluster expert by `1` for home clients
and `1/(K−1)` for non-home clients.

At inference time the router is forced to the one-hot home configuration, the
universal expert is deactivated, and the local classifier is replaced by the
cluster classifier of the target task.
...
As an upstream sanity check, the same code reproduces the original FedLEASE
four-task GLUE result at 87.88 average over five seeds, matching the 87.76
reported by upstream FedLEASE within seed noise.

## Quick Start

```bash
# 1) Sync the environment (uv recommended; see "Installation" for alternatives)
uv sync

# 2) Smoke check (~3 minutes on any CUDA GPU)
...
# 3) Reproduce the five-task dual-forward main result, seed 42
uv run python -u main.py \
  ...
  --save_final_params --cross_eval --seed 42 \
  --output_dir ./output/dualforward_5task_seed42
```

Repeat step 3 with `--seed 43`, `--seed 44` to reach the headline `74.49`
cross-target mean.
...
Python 3.11+. All commands must be run from the repository root so that the
in-tree `peft/` package is imported, not the public PEFT package.
...
A CUDA-enabled GPU is required for the reported runs. CPU execution is not
supported because RoBERTa-Large forward/backward dominates the runtime.
...
On each batch `(x, y)` at client `c_i`:
...
Backpropagation routes gradients asymmetrically:
...
The router is overridden to the one-hot home configuration; the universal
expert is deactivated; the local classifier `h_i` is replaced by the cluster
classifier `\bar h_{home(j)}` of the target task `T_j`, which the gateway
provides given the known task identity.

---

## Reproducing the Main Results

### 1) Four-task FedLEASE baseline (≈ 3 GPU-hours total)
...
### 2) Five-task dual-forward headline result (≈ 3 GPU-hours total)

Repeat with `--seed 42`, `--seed 43`, `--seed 44` to reach the headline
`74.49` cross-target mean.
...
Every script below reads `final_params.pt` produced by Section 1 / 2 and
...
The most relevant flags accepted by `main.py`. The full list is available
via `python main.py --help`.
...
Each post-hoc evaluation script under `scripts/eval/` writes a single JSON
file whose schema matches the ablation it implements.
...
contributions in this repository — the dual-forward objective, the
asymmetric server aggregation rule, and the post-hoc evaluation pipelines —
